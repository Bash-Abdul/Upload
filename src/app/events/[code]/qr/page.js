...
// Wrapper to handle margin without passing invalid prop to DOM
function QRCodeWithMargin({ includeMargin, ...props }) {
    const padding = includeMargin ? '16px' : '0'
    return (
        <div style={{ background: '#fff', padding, display: 'inline-block' }}>
            <QRCode {...props} />
        </div>
    )
}
...
export default function EventQR() {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            setUploadUrl(`${window.location.origin}/upload/${eventCode}`)
        }
    }, [eventCode])
